...
#单词编码
# 这里调用的map其实可以直接接着ds_train_raw上面的.map(split_line,num_parallel_calls = tf.data.experimental.AUTOTUNE)后面继续map
# 我理解这些map操作只会进行一次，也就是说vectorize_layer只会被调用1次，得到的token的int map就不变了
# 后续多个epoch遍历这个数据时，shuffle的已经是map过的token int了，也就不用担心不同epoch的map int不一致问题了。
