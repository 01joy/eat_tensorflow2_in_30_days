...
ds_train = tf.data.Dataset.list_files("./data/cifar2/train/*/*.jpg") \
           ...
           .shuffle(buffer_size = 1000).batch(BATCH_SIZE) \ # shuffle每个epoch会shuffle成不同的顺序，具体看tf api
...
for i,(img,label) in enumerate(ds_train.unbatch().take(9)): #ds_train原来已经按batch组织好了，比如[[batch1],[batch2],...]，unbatch就是去batch化，变成[img1,img2,...]
...
```python
for x,y in ds_train.take(1): # 这里读取的是batch化的ds_train，batchsize=100，img size是(32,32)，彩色3通道
...
```python
#在tensorboard中查看模型
notebook.start("--logdir=%s"%logdir)#必须加等号
```
